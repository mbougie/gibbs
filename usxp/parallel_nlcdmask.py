...
def createReclassifyList():
	...
	query = 'SELECT "Value",mtr from pre.{} as a JOIN pre.{} as b ON a.traj_array = b.traj_array WHERE mtr = 3'.format(data['pre']['traj']['filename'], data['pre']['traj']['lookup'])
	print 'query:', query
...
def execute_task(in_extentDict):
	...
	for row in getthelist:
		...
		for pixel_location in stacked_indices:
			...
			#get the length of nlcd list containing only yhe value 82
			count_82 = nlcd_list.count(82)
			
			if data['refine']['operator'] == 'or':
				if count_82 > 0:
					outData[row,col] = 250


			elif data['refine']['operator'] == 'and':
