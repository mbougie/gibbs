...
def createMetaTable(gdb_path, wc):
    ...
    for fc in featureclasses: 
        ...
        cur = conn.cursor()
        query="INSERT INTO intact_lands.metatable VALUES (" + str(fnf[2]) + " , " + str(fnf[3]) + " , '" + str(fnf[4]) + "' , '" + str(fnf[5])+ "')"
        print query
        cur.execute(query)
        conn.commit()


def main_layers(gdb_path, wc, year):
    arcpy.env.workspace = defineGDBpath(gdb_path)
    ...
    for fc in featureclasses: 
        ...
        #create layer for each shapefile
        createLayer(gdb_path, fc, layerlist)
    
    print len(layerlist)

    if len(layerlist) >= 1:
 
        arcpy.Merge_management(layerlist, defineGDBpath(['intact_land','merged'])+'merged_'+str(year))
...
print years
for year in years:
    main_layers(['intact_land','ia4'],'*_acea_'+str(year)+'*',year)
    deleteLayers()
...
def ProjectSF(infc, outfc):
    print 'ffff', outfc
    arcpy.env.workspace = defineGDBpath(['intact_land','ia4'])

    # proj =  "PROJCS['Albers_Conical_Equal_Area',GEOGCS['GCS_North_American_1983',DATUM['D_North_American_1983',SPHEROID['GRS_1980',6378137.0,298.257222101]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]],PROJECTION['Albers'],PARAMETER['false_easting',0.0],PARAMETER['false_northing',0.0],PARAMETER['central_meridian',-96.0],PARAMETER['standard_parallel_1',29.5],PARAMETER['standard_parallel_2',45.5],PARAMETER['latitude_of_origin',23.0],UNIT['Meters',1.0]]", "", "PROJCS['NAD_1983_UTM_Zone_15N',GEOGCS['GCS_North_American_1983',DATUM['D_North_American_1983',SPHEROID['GRS_1980',6378137.0,298.257222101]],PRIMEM['Greenwich',0.0],UNIT['Degree',0.0174532925199433]],PROJECTION['Transverse_Mercator'],PARAMETER['False_Easting',500000.0],PARAMETER['False_Northing',0.0],PARAMETER['Central_Meridian',-93.0],PARAMETER['Scale_Factor',0.9996],PARAMETER['Latitude_Of_Origin',0.0],UNIT['Meter',1.0]]"

    # Set output coordinate system
    # outCS = arcpy.SpatialReference(proj)

    # run project tool
    # arcpy.Project_management(infc, outfc, proj)
...
def main(state):
    ...
    for file in glob.glob("*.shp"):
        ...
        print outfc
        try:
            ProjectSF(file, outfc)
        except:
            print 'bad file:', file
            e = sys.exc_info()[1]
            
            query="INSERT INTO metadata.corrupt VALUES ('" + str(file) + "','"+str(fnf[2])  + "','"+str(e.args[0]) + "')"
            print query
            executeQuery(query)
