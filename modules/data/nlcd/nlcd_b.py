...
arcpy.env.workspace = gdb_nlcd

####get nlcd rasters in gdb
for nlcd in arcpy.ListRasters("*92*", "GRID"):
	print(nlcd.lower())
	###step1### --- add attribute tables to postgres
	
	# gen.addGDBTable2postgres_raster(gdb=gdb_nlcd, pgdb=pgdb, schema='nlcd', intable=nlcd, outtable='lookup_{}'.format(nlcd.lower()))

    ###step2### --- run sql script to add binary column to each lookup table

	###step3### --- reclassify rasters based on the binary column in lookup tables
	reclasslist = gen.createReclassifyList(pgdb=pgdb, query='SELECT value, b FROM nlcd.lookup_{} WHERE b in (0,1)'.format(nlcd.lower()))
	reclassifyRaster(gdb_in=gdb_nlcd, inraster=nlcd, gdb_out=gdb_nlcd_b, outraster='{}_b'.format(nlcd.lower()), reclasslist=reclasslist)
