...
create table test.clip_19073 as 

SELECT clipped.atlas_stco, ST_Union(ST_SnapToGrid(ST_Buffer(clipped_geom, 50),0.0001)) as geom
FROM 
...
WHERE ST_Dimension(clipped.clipped_geom) = 1 and clipped.atlas_stco = '19073'
GROUP BY clipped.atlas_stco;


ALTER TABLE test.clip_19073 ALTER COLUMN geom TYPE geometry(MultiPolygon,102003) USING ST_SetSRID(geom,102003);
...
FROM spatial.counties_102003 as a, test.clip_19073 as b
WHERE a.wkb_geometry && b.geom AND a.atlas_stco = '19073'