...
def step1():
    ...
    ###export intact_conversion to postgres
    # gen.addGDBTable2postgres_raster(gdb='D:\\projects\\intactland\\intact_clu\\final\\intactlands_new.gdb', pgdb='intact_lands', schema='intact_conversion', table='intact_conversion')

    ##export intact_conversion_cdl2015_nlcd2011 to postgres
    gen.addGDBTable2postgres_raster(gdb='D:\\projects\\intactland\\intact_clu\\final\\intactlands_new.gdb', pgdb='intact_lands', schema='intact_conversion', table='intact_conversion_cdl2015_nlcd2011')
...
def step3():
	...
	# create the nlcd_clu_intact_conversion
...
####################################################################################
##### combine rasters of different years for comparison  ##########################
##################################################################################

#### get input rasters that will be used as input for the combine function
gdb = 'D:\\intactland\\intact_clu\\final\\current.gdb'
arcpy.env.workspace = gdb

input_rasters = ['intactland_11_refined', 'intactland_15_refined']
outraster = "intact_conversion_11and15"



#### combine the rasters and then save them in a different gdb
# outCombine = Combine(input_rasters)
gdb = 'D:\\intactland\\intact_conversion\\intact_conversion.gdb'
arcpy.env.workspace = gdb
...
##########################################################
#### create zonal histogram dataset by county
##################################################################
counties = 'E:\\data\\shapefiles.gdb\\counties'
...
for raster in arcpy.ListRasters('*'):
	...
	### create zonal histogram dataset by county
	ZonalHistogram(in_zone_data=counties, zone_field='atlas_stco', in_value_raster=raster, out_table='{}_hist_counties'.format(raster))
	### export zonal histogram table to postgres
	gen.addGDBTable2postgres_histo_counties(gdb=gdb, pgdb='intactland', schema='intact_conversion', table='{}_hist_counties'.format(raster))
