...
setwd('I:\\d_drive\\projects\\side_projects\\nwf\\tif')
...
########## intact grasslands ###########################################
r1_init = raster('intact_grasslands_2017_agg_3km_sum.tif')
r1 = r1_init
hist(r1_init)
...
##### RELATIVE ##########
########## converted intact grasslands #############################
r2_init = raster('planted_grasslands_2017_agg_3km_sum.tif')
r2 = r2_init
...
m <- c(0, 5, 0,
       ...
       20, 40, 3,
       40, 80, 4,
       80, 10000000000000000, 5)   ####weird final value because of the issues of intactlands and conversion not being completely compatable datasets
rclmat <- matrix(m, ncol=3, byrow=TRUE)
...
r3_df$value[r3_df$value == 2] <- 12
r3_df$value[r3_df$value == 3] <- 13
r3_df$value[r3_df$value == 4] <- 14
r3_df$value[r3_df$value == 5] <- 15



r3_df$value[r3_df$value == 10] <- 11
r3_df$value[r3_df$value == 20] <- 21
r3_df$value[r3_df$value == 30] <- 31
r3_df$value[r3_df$value == 40] <- 41
r3_df$value[r3_df$value == 50] <- 51
...
legend_y_numeric <- getggplotObject_y(x = 0.038, y=0.060, multiplier = 0.045, slots = 4, labels = c("5","10","20","40","80"))
legend_y = annotation_custom(grobTree(textGrob("% Intact \U2192",  x=0.013, y= 0.16, rot = 90,gp=gpar(col="#4e4d47", fontsize=50, fontface="bold"))))



##### CONVERSION X-AXIS legend #################################################################
legend_x_numeric <- getggplotObject_x(x = 0.0525, y=0.03, multiplier = 0.0322, slots = 5, labels = c("5","10","20","40","80"))
legend_x = annotation_custom(grobTree(textGrob("% Planted \U2192", x=0.135, y= -0.01, rot = 0,gp=gpar(col="#4e4d47", fontsize=50, fontface="bold"))))
...
fileout = 'I:\\d_drive\\projects\\side_projects\\nwf\\deliverables\\test.png'
