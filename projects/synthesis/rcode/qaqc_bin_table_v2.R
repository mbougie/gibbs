...
getSummaryStats <- function(df, obj, figure_params, col, title){
  ...
  colnames(m) <- c("bins", "count")
  # fileout = gsub("figure", figure, "H:\\new_data_8_18_19\\d_drive\\synthesis\\s35\\deliverables\\current_dataset.csv")
  fileout = paste("H:\\new_data_8_18_19\\d_drive\\synthesis\\s35\\deliverables\\bin_values\\",obj$dataset,".csv",sep="")
  
  write.csv(m,fileout,row.names = F)
  sum_count = sum(m$count)
  ...
  bins <- apply(penguins, 1, paste, collapse=".........") ###convert columns into a string
  print(bins)
  # df_stats <- data.frame(col=col, title=title, min=min, q1=q1, q2=q2, q3=q3, max=max, bins=I(list(bin_breaks)))
  df_stats <- data.frame(col=col, title=title, sum_count=sum_count, min=min, q1=q1, q2=q2, q3=q3, max=max)
  print('----------------------df--------------------------------------')
  print('df_stats------------------------yo--------------------------------------------')
  print(str(df_stats))
  print('meow')
...
write.csv(summary_table,"H:\\new_data_8_18_19\\d_drive\\synthesis\\s35\\deliverables\\metadata\\test.csv",row.names = F)
