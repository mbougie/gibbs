...
createMap <- function(con, jsondata, filename, current_ds){
  
  
  ###create global variables
  
  legend_title = jsondata$intensification$map[[filename]]$legend_title
  print('-----------------legendtitle------------------')
  print(legend_title)
  bin_breaks = jsondata$intensification$map[[filename]]$bin_breaks
  print(bin_breaks)
  legend_labels = jsondata$intensification$map[[filename]]$legend_labels
  print(legend_labels)
  legend_range = jsondata$intensification$map[[filename]]$legend_range
  print(legend_range)
  title = jsondata$intensification$map[[filename]]$title
  print(title)
  legend_position = jsondata$intensification$map[[filename]]$legend_position
  print(legend_position)
  title_position_hjust = jsondata$intensification$map[[filename]]$title_position_hjust
  print(title_position_hjust)
  title_position_vjust = jsondata$intensification$map[[filename]]$title_position_vjust
  print(title_position_vjust)
  legend_label_vjust = jsondata$intensification$map[[filename]]$legend_label_vjust
  print(legend_label_vjust)
  legend_label_hjust = jsondata$intensification$map[[filename]]$legend_label_hjust
  print(legend_label_hjust)
  dataset = jsondata$intensification$map[[filename]]$dataset
  print(dataset)  
...
postgres_query <- dbGetQuery(con, gsub("current_field", filename, "SELECT atlas_stco, current_field as percent FROM intensification_agroibis.rfs_intensification_results_counties WHERE current_field IS NOT NULL;"))
...
d = ggplot() + 
  ...
  theme(
    ...
    plot.title = element_text(size= 25, vjust=title_position_vjust, hjust=title_position_hjust, color = "#4e4d47"),
