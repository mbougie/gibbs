...
# json_legend_file <- paste(rootpath, 'shemas\\seth\\json\\json_legends.json', sep='\\')
...
createDummy <- function(obj){
  ...
  d = ggplot(df, aes(x = x, y = y, fill = cuts)) +
    ...
    theme(
      ...
      legend.position = c(0.33, 0.5),   ####(horizontal, vertical)
  ...
  getggplotObject <- function(cnt, multiplier, slots, labels){
    ...
    while (cnt < limit) {
      print(cnt)
      ggplot_object = annotation_custom(grobTree(textGrob(labels[i], x=cnt, y= 0.5, just="left", rot = -45,gp=gpar(col="#4e4d47", fontsize=35, fontface="bold"))))
  ...
  legend_title <- annotation_custom(grobTree(textGrob(obj$legend_title, x = 0.33, y = 0.90, just="left", rot = 0,gp=gpar(col="#4e4d47", fontsize=45, fontface="bold"))))
  legendlabels <- getggplotObject(cnt = 0.33, multiplier = 0.035, slots = length(labels), labels = labels)
...
# createDummy(json_legend$intensification$map$phos)
