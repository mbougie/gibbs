...
getQuery <- function(){
  if(dataset == "acres"){
    ...
    return(dbGetQuery(con, gsub("current_field", filename, "SELECT fips, current_field as percent FROM synthesis_extensification.agroibis_counties WHERE current_field IS NOT NULL;")))}
...
getMapDF <- function(){
  if(dataset == "acres"){
    ...
    current <- readOGR(dsn = "D:\\projects\\synthesis\\s35\\extensification\\shapefiles", layer = "extensification_agroibis_wgs84")
    # current
    current@data$id <- rownames(current@data)
    current.df     <- fortify(current)
    current.df <- join(current.df, current@data, by="id")
