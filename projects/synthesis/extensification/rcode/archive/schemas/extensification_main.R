...
### Expansion:attach df to specific object in json #####################################################
jsondata$mlra$ratio_expand_rfs_mlra$df <- get_postgis_query(con_synthesis, "SELECT ratio_expand_rfs_mlra as current_field, st_transform(geom,4326) as geom FROM extensification_ksu.extensification_mlra WHERE ratio_expand_rfs_mlra IS NOT NULL",
                                                geom_name = "geom")


### Abandonment:attach df to specific object in json #####################################################
jsondata$mlra$ratio_abandon_rfs_mlra$df <- get_postgis_query(con_synthesis, "SELECT ratio_abandon_rfs_mlra as current_field, st_transform(geom,4326) as geom FROM extensification_ksu.extensification_mlra WHERE ratio_abandon_rfs_mlra IS NOT NULL",
                                                geom_name = "geom")
