...
con_nri <- dbConnect(PostgreSQL(), dbname = 'synthesis', user = user, host = host, port=port, password = password)
mapa <- get_postgis_query(con_nri, "SELECT * FROM merged_t2",
                         geom_name = "geom")
...
geom_polygon(
  ...
  aes(y=lat, x=long, group=group, fill = lrr_group),
  colour = '#cccccc',
  size = 0.5
) 
