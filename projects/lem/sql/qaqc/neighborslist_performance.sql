...
create table v3.block_neighbors_list_t2 as 
SELECT 
a.src_geoid,
string_agg(a.nbr_geoid, ', '::text) AS neighbor_list
FROM v3.block_neighbors a
...
SELECT
main.src_geoid,
string_agg(main.nbr_geoid, ', '::text) AS neighbor_list
FROM 
(SELECT 
a.src_geoid,
a.nbr_geoid
...
WHERE a.length <> 0::double precision
limit 3000000) as main
GROUP BY main.src_geoid





CREATE TABLE v3.block_v2_table_qaqc_t2 AS 
 SELECT main.geoid,
   ...
   --------------------------------------------------------------------------------
    ...
    ---join biomes dataset
    LEFT JOIN v3.block_zonal_maj_biomes as biomes ON main.geoid::text = biomes.geoid;
