...
def getDeliverables(version, levellist):
    ...
    for level in levellist:
        ...
        convertPGtoFC(gdb=version, schema=version, table='{}_final_union'.format(level))
...
# getDeliverables('v3_3', ['county'])
# getDeliverables('v3_3', ['tract'])
# getDeliverables('v3_3', ['block_group'])
# getDeliverables('v3_3', ['block'])

def convertPGtoJSON_temp(version, pgdb, schema, table):
    # command = ["ogr2ogr.exe", "PostgreSQL", "PG:host=144.92.235.105 user=mbougie password=Mend0ta! dbname=lem", "D:\\projects\\lem\\lem.gdb", "-nlt", "PROMOTE_TO_MULTI", "-nln", "blocks.us_blck_grp_2016_mainland_5070", "us_blck_grp_2016_mainland_5070", "-progress", "--config", "PG_USE_COPY", "YES"]
    command = 'ogr2ogr -f "geojson" -progress I:\\d_drive\\projects\\lem\\projects\\main\\deliverables\\version\\v3\\{0}\\{3}_limit1000.json PG:"dbname={1} user=mbougie host=144.92.235.105 password=Mend0ta!" -sql "SELECT * FROM {2}.{3} LIMIT 1000" -t_srs EPSG:4152'.format(version, pgdb, schema, table) 
    print 'command:', command
    os.system(command)


################### QAQC ###################################
# convertPGtoFC(gdb='qaqc', schema='v3_3_final', table='county_final')
# convertPGtoFC(gdb='qaqc', schema='v3_3_final', table='block_final')
convertPGtoJSON_temp(version='v3_3', pgdb='lem', schema='v3_3_final', table='block_final')
