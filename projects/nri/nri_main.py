...
def createGrossTable():
	cur = conn.cursor()

	query = '''CREATE TABLE main.gross as 
			SELECT 
			  counties.atlas_st, 
			  counties.st_abbrev, 
			  counties.state_name, 
...
list_columns = ['07', '08', '09', '10', '11', '12']

def gross_abandonment():
	## create the base table by merging all states datasets from 2008 - 2016 together into one table
	cur = conn.cursor()

	i = 0
	while i < (len(list_columns)-1):

		query = ''' ALTER TABLE main.gross ADD COLUMN abandon_{1} integer;
		UPDATE main.gross
		SET abandon_{1}=subquery.sum*(100)
		FROM (SELECT 
		  nri12_cty_121115_subset.fips, 
		  sum(nri12_cty_121115_subset.xfact) as sum
		FROM 
		  main.nri12_cty_121115_subset
		WHERE broad{0} IN (1,2) AND broad{1} NOT IN (1,2,12)
		GROUP BY fips) AS subquery
		WHERE gross.atlas_stco=subquery.fips;'''.format(list_columns[i], list_columns[i+1])
...
def gross_crp():
	...
	while i < (len(list_columns)-1):
		...
		query = ''' ALTER TABLE main.gross ADD COLUMN crp_expansion_{1} integer;
		UPDATE main.gross
		SET crp_expansion_{1}=subquery.sum*(100)
		FROM (SELECT 
		  nri12_cty_121115_subset.fips, 
		  sum(nri12_cty_121115_subset.xfact) as sum
		FROM 
		  main.nri12_cty_121115_subset
		WHERE broad{0} NOT IN (1,2,12) AND broad{1} IN (12)
		GROUP BY fips) AS subquery
		WHERE gross.atlas_stco=subquery.fips;'''.format(list_columns[i], list_columns[i+1])
...
def gross_abandonment_year():
	engine = create_engine('postgresql://mbougie:Mend0ta!@144.92.235.105:5432/nri')
	df = pd.read_sql_query('SELECT sum(abandon_08) as cy_2008, sum(abandon_09) as cy_2009, sum(abandon_10) as cy_2010, sum(abandon_11) as cy_2011, sum(abandon_12) as cy_2012 FROM main.gross',con=engine)
	print df
	...
	df.to_sql('gross_abandonment_year', engine, schema='main')




def gross_abandonment_year_state():
	engine = create_engine('postgresql://mbougie:Mend0ta!@144.92.235.105:5432/nri')
	df = pd.read_sql_query('SELECT st_abbrev, sum(abandon_08) as cy_2008, sum(abandon_09) as cy_2009, sum(abandon_10) as cy_2010, sum(abandon_11) as cy_2011, sum(abandon_12) as cy_2012 FROM main.gross GROUP BY st_abbrev',con=engine)
	print df
	...
	df.to_sql('gross_abandonment_year_state', engine, schema='main')



def gross_crp_year():
	engine = create_engine('postgresql://mbougie:Mend0ta!@144.92.235.105:5432/nri')
	df = pd.read_sql_query('SELECT sum(crp_expansion_08) as cy_2008, sum(crp_expansion_09) as cy_2009, sum(crp_expansion_10) as cy_2010, sum(crp_expansion_11) as cy_2011, sum(crp_expansion_12) as cy_2012 FROM main.gross',con=engine)
	print df
	...
	df.to_sql('gross_crp_year', engine, schema='main')
...
def net_crop():
	...
	for column in list_columns:
		...
		FROM  main.nri12_cty_121115_subset
		WHERE nri12_cty_121115_subset.broad{0} <> 1 AND nri12_cty_121115_subset.broad{0} <> 2 AND nri12_cty_121115_subset.broad{0} <> 12 
		GROUP BY fips) AS subquery
...
def net_noncrop():
	...
	for column in list_columns:
		...
		FROM (SELECT fips, sum(broad{0}) as sum
		FROM  main.nri12_cty_121115_subset
		WHERE nri12_cty_121115_subset.broad{0} = 1 or nri12_cty_121115_subset.broad{0} = 2
		GROUP BY fips) AS subquery
		WHERE base.atlas_stco=subquery.fips;'''.format(column)
