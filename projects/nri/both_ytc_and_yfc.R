...
df1 <- dbGetQuery(con1, "SELECT year::integer, acres FROM main.gross_abandonment_year")
# df2 <- dbGetQuery(con1, "SELECT year::integer, acres FROM main.gross_crp_year")
df3 <- dbGetQuery(con2, "SELECT years,acres,series,yxc,series_order FROM counts_yxc.merged_series WHERE yxc='yfc' AND series IN ('s1','s25','s27','s32','s33', 's34', 's35') ")


##this reorders the labels in the legend in chronological order
df3$series <- with(df, reorder(series, series_order))
...
plot = ggplot(NULL) +
  ...
  # geom_line(data = df2, linetype="dashed", aes(x=year, y=acres)) +
  geom_line(data = df3, aes(x=years, y=acres, group=series_order, color=series, ordered = TRUE)) + 
