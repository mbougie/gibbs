...
def createReclassifyList(data):
	...
	query = "SELECT \"Value\" from pre.{} as a JOIN pre.{} as b ON a.traj_array = b.traj_array WHERE mtr=1".format(data['pre']['traj']['filename'], data['pre']['traj']['lookup_name'])
	print 'query:', query
	...
	for row in traj_list:
		...
		for pixel_location in stacked_indices:
			...
			if data['refine']['mask_nlcd']['operator'] == 'or':
				...
				#### needs to now have 2 years of non 81 or 82 for both years to be considered a false conversion  (i.e. no crop there so how can there be abandonment when no crop exists!!!)


				###if not NLCD both years and mtr1 is true than treat the pixel as false conversion
				elif count_82 < 2 and traj != None:
