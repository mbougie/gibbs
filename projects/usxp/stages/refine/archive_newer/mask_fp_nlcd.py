...
def labelPixel(data, ytc, yfc, count_82, count_81_82):
	...
	if count_82 > 0 and ytc != None:
		return data['refine']['arbitrary_crop']
	...
	#### needs to now have to years of non 81 or 82 for both years to be considered a false conversion  (i.e. no crop there so how can there be abandonment when no crop exists!!!)
		...
		return data['refine']['arbitrary_noncrop']
...
def run(data):
	...
	for row in arcpy.da.SearchCursor('C:\\Users\\Bougie\\Desktop\\Gibbs\\data\\usxp\\ancillary\\vector\\shapefiles.gdb\\fishnet_nlcd_1tile', ["oid","SHAPE@"]):
		atlas_stco = row[0]
		print atlas_stco
