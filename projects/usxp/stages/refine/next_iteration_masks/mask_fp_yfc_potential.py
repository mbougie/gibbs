...
def createReclassifyList(data):
	...
	query = "SELECT \"Value\", mtr, a.traj_array from pre.{} as a JOIN pre.{} as b ON a.traj_array = b.traj_array WHERE potential_yfc = 1".format(data['pre']['traj']['filename'], data['pre']['traj']['lookup_name'])
	print 'query:', query
...
def execute_task(args):
	...
	for row in traj_list:
		...
		for pixel_location in stacked_indices:
			...
			###  if NLCD both years and then treat pixel as conversion
			if (count_82 != 2):
				outData[row,col] = data['refine']['arbitrary_mtr5']
...
def mosiacRasters(data):
	...
	arcpy.MosaicToNewRaster_management(tilelist, data['refine']['gdb'], data['refine']['mask_fp_yfc_potential']['filename'], inTraj.spatialReference, "16_BIT_UNSIGNED", 30, "1", "LAST","FIRST")


	#Overwrite the existing attribute table file
	arcpy.BuildRasterAttributeTable_management(data['refine']['mask_fp_yfc_potential']['path'], "Overwrite")

	# Overwrite pyramids
	gen.buildPyramids(data['refine']['mask_fp_yfc_potential']['path'])
