...
def reclassifyRaster():
    ...
    outraster = raster.replace("_", "_noncrop_")
...
def getReclassifyValuesString(ds, reclass_degree):
    ...
    query = "SELECT value::text,{0} FROM misc.lookup_{1} WHERE {0} = '0' AND value NOT IN (121,122,123,124) ORDER BY value".format(reclass_degree, ds)
    print query
