...
getQuery <- function(){
  if(dataset == "nass"){
    return(dbGetQuery(con, gsub("current_field", filename, "SELECT atlas_stco, perc_conv_county as percent FROM ag_census.ag_census_expansion;")))}
...
mapa.df <- st_read(dsn,  query = "SELECT atlas_stco, perc_conv_county as percent, geom FROM ag_census.ag_census_expansion")
