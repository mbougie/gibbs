...
legend <- ggplot(total, aes(x=name, y=acres, fill=name)) +
  ...
  theme(
        ...
        axis.text.y=element_text(size=30), #### change text size on y-axis
        
        ### x-axis attribute
        axis.title.x=element_text(size=35),
        axis.text.x=element_text(size=30, margin = margin(t = 0, r = 0, b = 20, l = 0)) #### change text size on x-axis
  )+

  labs(y="Acreage in Millions", x="Suitability classes")+
  ...
  ####remove gap between label and bar
