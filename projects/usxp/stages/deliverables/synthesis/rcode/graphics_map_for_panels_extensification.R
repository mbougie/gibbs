...
getQuery <- function(){
  if(dataset == "acres"){
    ...
    return(dbGetQuery(con, gsub("current_field", filename, "SELECT fips, current_field as percent FROM synthesis_extensification.agroibis WHERE current_field IS NOT NULL;")))}
...
getMapDF <- function(){
  if(dataset == "acres"){
    current <- readOGR(dsn = "D:\\projects\\usxp\\deliverables\\maps\\synthesis\\extensification\\shapefiles", layer = "extensification_mlra_wgs84")
    # current
    ...
    current <- readOGR(dsn = "D:\\projects\\usxp\\deliverables\\maps\\synthesis\\extensification\\shapefiles", layer = "extensification_agroibis_wgs84")
    # current
    current@data$id <- rownames(current@data)
    current.df     <- fortify(current)
    current.df <- join(current.df, current@data, by="id")
...
getFillValues <- function(filename, legend_range){
  print(filename)
  v <- c('ratio_abandon_rfs_mlra', 'gal_et_aban_imp_rfs', 's_gigagrams_co2e')
