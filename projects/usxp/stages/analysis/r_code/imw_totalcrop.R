...
df <- dbGetQuery(con, "SELECT year, mtr2 as acres FROM counts_imw.merged_imw_pvt;")
...
plot = ggplot(df, aes(x=year, y=acres, ordered = TRUE)) +
  ...
  ggtitle('null') + theme(plot.title = element_text(hjust = 0.5)) +
...
p1 <- plot + geom_text(x = 2010.5, y = 320000000, label = lm_eqn(df), parse = TRUE)
p1
