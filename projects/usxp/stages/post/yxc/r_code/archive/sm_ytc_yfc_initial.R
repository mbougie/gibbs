...
df_35 <- dbGetQuery(con2, "SELECT b.st_abbrev as state,a.year::int,a.acres::int FROM zonal_hist.s33_zonal_hist_table as a, spatial.states as b WHERE b.atlas_st = a.atlas_st;")
...
plotResult<-ggplot(NULL, aes(x=year, group=state)) +
  ...
  geom_line(data = df_35, aes(y=acres,color="s33"),alpha=0.8)+
