...
df <- dbGetQuery(con, "SELECT b.group_name as name, b.group_color as color, sum(a.acres) as acres, a.year FROM combine.s35_conus_ytc_fc a INNER JOIN misc.lookup_cdl b ON a.label=b.value WHERE color IS NOT NULL AND group_name IN ('Corn', 'Soybeans', 'Alfalfa', 'Wheat', 'Cotton', 'Sorghum') GROUP BY b.group_name, b.group_color, a.year")
