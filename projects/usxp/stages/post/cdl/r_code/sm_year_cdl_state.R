...
df <- dbGetQuery(con, "SELECT b.group_name as name, b.group_color as color, sum(a.acres) as acres, a.year, c.st_abbrev as state FROM  combine.s35_sm_yfc_bfnc a INNER JOIN misc.lookup_cdl b ON a.label=b.value INNER JOIN spatial.states as c ON a.state=c.atlas_st WHERE color IS NOT NULL AND group_name IN ('Corn', 'Soybeans', 'Alfalfa', 'Wheat', 'Cotton', 'Sorghum') GROUP BY b.group_name, b.group_color, a.year, c.st_abbrev order by year, name, state")
